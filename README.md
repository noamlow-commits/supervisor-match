...
### זרימת עבודה (v2.5)
1. בתחילת המחזור: המזכירה מדביקה את רשימת התלמידים (שם בכל שורה, או "שם, מייל, טלפון")
2. עם כל שיבוץ: לוחצת על dropdown ובוחרת את המדריך — הסטטוס מתעדכן מיד
3. בכל רגע נתון: כרטיס "תלמידים ממתינים" מציג מספר + מי הם בשמות
